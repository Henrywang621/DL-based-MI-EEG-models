    ...
    if group_id == 1:
        for subject_id in track(subject_ids, description = 'Processing...'):
            for MI_run in runs_type2:
                ...
                # discard the last trial in the runs
                for event in events[:-1]:
                    if event[2] == 'T0':
                        continue
                    else:
                        x = segment_X(event, data[indices,:], istraining)
            ...
            for MI_run in runs_type2 + runs_type3:
                ...
                for event in events:
                    if MI_run in runs_type2 and event[2] == 'T1':
                        x, y = generate_data(event, data[indices,:], istraining, label = 0)
                        crops += x
                        labels += y
                    elif MI_run in runs_type3 and event[2] == 'T2':
                        x, y = generate_data(event, data[indices,:], istraining, label = 1)
                        crops += x
                        labels += y
