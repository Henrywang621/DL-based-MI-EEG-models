...
def segment_X(event, data, istraining):
    ...
    x = [mne.filter.resample(data[:, windows[n][0]: windows[n][1]], up=1.5625, npad='auto') 
...
def load_data(subject_ids, group_id, istraining):
    ...
    if group_id == 1:
        ...
        raise ValueError('group_id should be in the range of 1 to 10 !!!')
    
    return crops, labels 

def prepare_data(X, y, isonehot = False):
