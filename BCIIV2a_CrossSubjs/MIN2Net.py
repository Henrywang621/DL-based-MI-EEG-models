...
from loss import mean_squared_error, triplet_loss, SparseCategoricalCrossentropy
from utils import TimeHistory, compute_class_weight
import os


class MIN2Net:
    def __init__(self,
                input_shape=(1,400,22), 
                num_class=4, 
    ...
    def fit(self, X_train, y_train, X_val, y_val):
        ...
        model.fit(x=X_train, y=[X_train,y_train,y_train],
                  batch_size=self.batch_size, shuffle=self.shuffle,
                  epochs=self.epochs, validation_data=(X_val, [X_val,y_val,y_val]),
                  callbacks=[es, reduce_lr,es])
        model.save(self.weights_dir, save_format='tf')
