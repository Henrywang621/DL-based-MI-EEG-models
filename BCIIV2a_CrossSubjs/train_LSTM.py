...
for i in range(5):
    ...
    LSTM_model = LSTM('alstm')
    model = LSTM_model.create_model(input_shape=(samples, chans), 
                                        num_hidden_neurons=40, print_summary=True)
    weights_path = "Checkpoints/LSTM-{0}.hdf5".format(i+1)
    ...
    model = load_model('Checkpoints/LSTM-{0}.hdf5'.format(i+1))
...
np.save('Checkpoints/results_LSTM.npy', results)
