...
def split_datasets(seed = 42):
    random.seed(seed)
    
    # There are nine subjects totally. 
    subj_list = list(range(1,10))
    subjs4training = random.sample(subj_list, 6)
    for i in subjs4training:
       subj_list.remove(i)
    subjs4validation = random.sample(subj_list, 1)
    subj_list.remove(subjs4validation[0])

    return subjs4training, subjs4validation, subj_list
...
def std(data):
    for i in range(data.shape[0]):
        scaler = StandardScaler().fit(data[i, :, :])
