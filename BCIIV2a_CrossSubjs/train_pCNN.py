...
if gpus:
  ...
  try:
    tf.config.experimental.set_virtual_device_configuration(
        gpus[0],
        [tf.config.experimental.VirtualDeviceConfiguration(memory_limit=12000)]) # Notice here
...
for i in range(5):
    ...
    cnn_stft = CNN_STFT()
    model = cnn_stft.create_model(data_trX_c.shape[1:], dropout = DROPOUT, print_summary = False) 
    weights_path = "Checkpoints/pCNN-{0}.hdf5".format(i+1)
    ...
    fittedModel = model.fit(data_trX_c, data_trY_c, epochs = epochs, 
                            batch_size = batch_size, validation_data = 
                            (data_vaX_c, data_vaY_c), verbose= 2, shuffle = True, 
                            callbacks=callback_list)
    model = load_model('Checkpoints/pCNN-{0}.hdf5'.format(i+1), custom_objects={'Spectrogram': Spectrogram, 'Normalization2D': Normalization2D})
...
np.save('Checkpoints/results_pCNN.npy', results)
